...
Random／Driftから守りたいエフェクトは、見出しの鍵を閉じます。Random設定内のPattern Seedは、空欄または`-1`で自動更新、0以上の整数でシミュレーション系エフェクトの破壊配置を固定します。`New Seed`は新しい固定Seedを生成します。Region Maskは矩形に加えて、暗部・中間調・明部・エッジ・ノイズを適用範囲にできます。
